...
function Class_C_Shop(){
	...
	this.BuyNow = function(){
		if(true==OpenWBS.C.Shop.AddToCart()){window.location.href="openwbs.client-1.js-mycart"/*tpa=http://www.cloudsmi.com/company/include/javascript/openwbs.client.js?mycart*/+filesuffix+"";}
			...
			for(var i=0;i<Fn_JSON.thepacks.length;i++){
				if(Fn_JSON.thepacks[i]!=null){
					if(Fn_JSON.thepacks[i].id==AdjunctID && Fn_JSON.thepacks[i].attr==AttrString && Fn_JSON.thepacks[i].selectgoodsid==AdjunctSelectGoodsID){
						...
						window.location.href="openwbs.client-1.js-mycart"/*tpa=http://www.cloudsmi.com/company/include/javascript/openwbs.client.js?mycart*/+filesuffix+"";
						return false;
					}
				}
			}
...
function Class_C_Cart(){
	...
	this.Order = function(){
		...
		if(OpenWBS.IsUserLogined){
			window.location.href="openwbs.client-1"/*tpa=http://www.cloudsmi.com/company/include/javascript/openwbs.client.js?mycart/checkout*/+filesuffix;
		}else{
	...
	this.PayForCart = function(){
		...
		if(Check){
			...
			form.action="orderpay-1.asp.htm"/*tpa=http://www.cloudsmi.com/company/include/javascript/include/module/payment/orderpay.asp*/;
		    form.submit();
		}else{
...
function Class_C_Guestbook(){
	...
	this.Post = function(){
		...
		var tel      = trim(OpenWBS.$id("guestbook-1.tel.htm"/*tpa=http://www.cloudsmi.com/company/include/javascript/guestbook.tel*/).value);
		...
		if(check){
			...
			$.ajax({
				...
				url:"ajax.guestbook-1.asp-ctl=guestbook&action=post.htm"/*tpa=http://www.cloudsmi.com/company/include/javascript/include/module/ajax/ajax.guestbook.asp?ctl=guestbook&action=post*/,
...
function Class_C_User(){
	...
	this.PostForgetPassword = function(){
		...
		if(Check){
			...
			$.ajax({
					type:"POST",dataType:"html",async:true,
					url:"ajax-1.asp-ctl=forgetpassword.htm"/*tpa=http://www.cloudsmi.com/company/include/javascript/include/module/ajax/ajax.asp?ctl=forgetpassword*/,data:ParaString,
	...
	this.ResetPassword = function(FormName){
		...
		if(Check){
			...
			$.ajax({
					type:"POST",dataType:"html",async:true,url:"ajax-1.asp-ctl=resetpassword.htm"/*tpa=http://www.cloudsmi.com/company/include/javascript/include/module/ajax/ajax.asp?ctl=resetpassword*/,data:ParaString,
					...
					success:function(ajaxText){
						if(ajaxText=="ok"){
							...
							window.setTimeout(function(){window.location.href="index-1.asp-login.htm"/*tpa=http://www.cloudsmi.com/company/include/javascript/index.asp?login*/+filesuffix},8000);
						}else if(ajaxText=="error_checkcode"){
							tip.className="d_err";
