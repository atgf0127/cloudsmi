...
function Detail_Init(){
	...
	if($("#artcontent pre").length>0){
		...
		window.onload = function(){
			...
			$(".code .line.htm"/*tpa=http://www.cloudsmi.com/template/default_widescreen/js/.code .line*/).wrap("<li></li>");
		}
	}
}
//更新用户账号状态
...
function PrintSearchHTML(PrintArea,SearchSystem){
	...
	if(objExist($area)){
		...
		var html = '<form action="index.asp-ctl=search.htm"/*tpa=http://www.cloudsmi.com/template/default_widescreen/js/index.asp?ctl=search*/ method="post">'+
